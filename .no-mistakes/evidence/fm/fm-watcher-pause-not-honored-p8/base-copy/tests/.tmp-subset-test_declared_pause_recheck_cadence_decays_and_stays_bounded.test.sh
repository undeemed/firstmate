...
# Same budget reasoning as wait_for_exit above, and for the same reason: the
# watcher does bounded startup work before its first poll, so a 3s budget reaps
# these waits while the watcher is still starting and reports a marker that never
# appeared. Observed as three different tests in this file failing on a loaded box,
# each on a 30-tick wait, none of them reproducible on an idle one. A generous
# budget can only remove that false negative - a marker the watcher never writes
# still fails when the budget runs out.
wait_numeric_file() {
...
test_classifier_primitives() {
	...
	dir=$(make_case classify-primitives)
	state="$dir/state"
	printf 'working: a\n\ndone: b\n\n' >"$state/x.status"
...
test_crew_is_provably_working_classifier() {
	local dir fakebin
	dir=$(make_case provably-working)
	fakebin="$dir/fakebin"
...
test_crew_worktree_written_since_classifier() {
	...
	dir=$(make_case classify-worktree-writes)
	state="$dir/state"
	anchor="$state/anchor"
	wt="$dir/wt"
	home="$dir/mate-home"
	statedir_wt="$dir/wt-with-state"
...
test_empty_write_prune_widens_the_probe() {
	...
	dir=$(make_case classify-empty-write-prune)
	state="$dir/state"
	anchor="$state/anchor"
	wt="$dir/wt"
...
test_empty_write_prune_from_the_environment_widens_the_probe() {
	...
	dir=$(make_case classify-empty-write-prune-env)
	state="$dir/state"
	anchor="$state/anchor"
	wt="$dir/wt"
...
test_worktree_write_probe_is_wall_clock_bounded() {
	...
	dir=$(make_case classify-write-probe-bound)
	state="$dir/state"
	anchor="$state/anchor"
	wt="$dir/wt"
	slowbin="$dir/slowbin"
...
test_signal_crew_provably_working_classifier() {
	...
	dir=$(make_case signal-provably-working)
	fakebin="$dir/fakebin"
	state="$dir/state"
	export FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh"
...
test_secondmate_status_signal_never_absorbed_classifier() {
	local dir fakebin state
	dir=$(make_case secondmate-signal-classify)
	fakebin="$dir/fakebin"
	state="$dir/state"
	export FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh"
...
test_provably_working_signal_absorbed() {
	...
	dir=$(make_case provably-working-signal)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher exited for a working: signal whose crew is provably working (should absorb): $(cat "$out")"
	fi
...
test_turn_ended_provably_working_absorbed() {
	...
	dir=$(make_case turn-ended-working)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher exited for a turn-end whose crew is provably working (should absorb): $(cat "$out")"
	fi
...
test_turn_ended_not_working_surfaced() {
	...
	dir=$(make_case turn-ended-stopped)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
...
test_working_note_not_working_surfaced() {
	...
	dir=$(make_case working-note-stopped)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
...
test_secondmate_status_note_surfaced_despite_busy_agent() {
	local dir state fakebin out drain_out pid
	dir=$(make_case secondmate-note-surfaced)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
...
test_self_announced_close_does_not_rewake_but_next_note_does() {
	...
	dir=$(make_case self-close-quiet)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	status_file="$state/task.status"
  ...
  ' _ "$ROOT/bin/fm-wake-lib.sh" "$state" "$status_file" || rc=$?
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "the home's own bookkeeping close re-woke its own watcher: $(cat "$out")"
	fi
	[ ! -s "$out" ] || {
		reap "$pid"
		fail "self-announced close printed a wake reason: $(cat "$out")"
	}
	[ ! -s "$state/.wake-queue" ] || {
		reap "$pid"
		fail "self-announced close enqueued a durable wake"
	}
...
test_actionable_signal_surfaced() {
	local dir state fakebin out drain_out status_file pid
	dir=$(make_case actionable-signal)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
...
test_terminal_stale_surfaced() {
	...
	sig=$(seen_sig "$state/done.status")
	printf '%s' "$sig" >"$state/.seen-done_status"
...
test_stale_terminal_status_overridden_by_active_run() {
	local dir state fakebin out drain_out capture_file window key pane_hash sig pid
	dir=$(make_case terminal-stale-overridden)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	...
	sig=$(seen_sig "$state/validating.status")
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher exited for a stale terminal-looking status the run-step overrides (should absorb): $(cat "$out")"
	fi
...
test_nonterminal_stale_provably_working_absorbed_then_escalated() {
	local dir state fakebin out drain_out capture_file window key pane_hash sig pid
	dir=$(make_case nonterminal-stale-working)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	...
	sig=$(seen_sig "$state/quiet.status")
	printf '%s' "$sig" >"$state/.seen-quiet_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher exited for a fresh provably-working non-terminal stale (should absorb): $(cat "$out")"
	fi
...
test_nonterminal_stale_not_working_surfaced() {
	local dir state fakebin out drain_out capture_file window key pane_hash sig pid
	dir=$(make_case nonterminal-stale-stopped)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
...
test_nonterminal_stale_paused_absorbed_then_resurfaced() {
	...
	dir=$(make_case nonterminal-stale-paused)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher exited for a fresh declared pause (should absorb): $(cat "$out")"
	fi
...
test_declared_pause_is_bounded_for_exited_and_live_agents() {
	...
	dir=$(make_case exited-captain-held)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
...
test_secondmate_paused_resurfaces_in_normal_mode() {
	...
	dir=$(make_case secondmate-paused-resurface)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	statusf="$state/secondmate-held.status"
	...
	sig=$(seen_sig "$statusf")
	printf '%s' "$sig" >"$state/.seen-secondmate-held_status"
	key=$(printf '%s' "$window" | tr '.:/' '___')
...
test_secondmate_captain_held_resurfaces_in_normal_mode() {
	local dir state fakebin out capture_file statusf window key pane_hash sig pid back
	dir=$(make_case secondmate-held-resurface)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	statusf="$state/secondmate-hold.status"
	...
	sig=$(seen_sig "$statusf")
	printf '%s' "$sig" >"$state/.seen-secondmate-hold_status"
	key=$(printf '%s' "$window" | tr '.:/' '___')
...
test_secondmate_nonpaused_stale_remains_suppressed() {
	...
	dir=$(make_case secondmate-stale-suppressed)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	statusf="$state/secondmate-working.status"
	...
	sig=$(seen_sig "$statusf")
	printf '%s' "$sig" >"$state/.seen-secondmate-working_status"
	key=$(printf '%s' "$window" | tr '.:/' '___')
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher surfaced an ordinary secondmate stale pane: $(cat "$out")"
	fi
	[ ! -s "$out" ] || {
		reap "$pid"
		fail "ordinary secondmate stale pane printed a wake reason: $(cat "$out")"
	}
	reap "$pid"
...
test_secondmate_unpause_clears_pause_tracking() {
	...
	dir=$(make_case secondmate-unpause-clears)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	statusf="$state/secondmate-resumed.status"
	window="test:fm-secondmate-resumed"
	...
	[ ! -e "$state/.paused-$key" ] || {
		reap "$pid"
		fail "resumed secondmate retained the pause marker"
	}
	[ ! -e "$state/.stale-$key" ] || {
		reap "$pid"
		fail "resumed secondmate retained stale tracking"
	}
	[ ! -e "$state/.wedge-escalations-$key" ] || {
		reap "$pid"
		fail "resumed secondmate retained wedge tracking"
	}
	reap "$pid"
...
test_nonterminal_stale_pause_transitions_reclassify_unchanged_hash() {
	...
	dir=$(make_case nonterminal-stale-pause-transition)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-transition"
	printf 'idle awaiting external\n' >"$capture_file"
	printf 'window=%s\nkind=ship\n' "$window" >"$state/transition.meta"
	printf 'paused: awaiting the upstream release\n' >"$state/transition.status"
	sig=$(seen_sig "$state/transition.status")
	printf '%s' "$sig" >"$state/.seen-transition_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle awaiting external")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '%s' "$pane_hash" >"$state/.stale-$key"
	...
	[ -e "$state/.paused-$key" ] || {
		reap "$pid"
		fail "unchanged stale hash did not enter paused mode"
	}
	[ ! -e "$state/.stale-since-$key" ] || {
		reap "$pid"
		fail "pause transition retained its wedge timer"
	}
	...
	kill -0 "$pid" 2>/dev/null || {
		reap "$pid"
		fail "a stale hash that left pause did not resume wedge tracking: $(cat "$out")"
	}
	[ ! -e "$state/.paused-$key" ] || {
		reap "$pid"
		fail "unchanged stale hash retained paused mode after resume"
	}
	[ -s "$state/.stale-since-$key" ] || {
		reap "$pid"
		fail "unchanged stale hash did not restart wedge tracking after resume"
	}
	wait_poll_cycle "$state" "$pid" || {
		reap "$pid"
		fail "a stale hash that left pause did not resume wedge tracking: $(cat "$out")"
...
test_nonterminal_paused_rechecks_authoritative_state() {
	...
	dir=$(make_case nonterminal-paused-recheck)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-pause-recheck"
	printf 'idle awaiting external\n' >"$capture_file"
	printf 'window=%s\nkind=ship\n' "$window" >"$state/pause-recheck.meta"
	printf 'paused: awaiting the upstream release\n' >"$state/pause-recheck.status"
	sig=$(seen_sig "$state/pause-recheck.status")
	printf '%s' "$sig" >"$state/.seen-pause-recheck_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle awaiting external")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '%s' "$pane_hash" >"$state/.stale-$key"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "an active run behind a declared pause surfaced instead of resuming wedge tracking: $(cat "$out")"
	fi
	[ ! -e "$state/.paused-$key" ] || {
		reap "$pid"
		fail "authoritative active run retained paused mode"
	}
	[ -s "$state/.stale-since-$key" ] || {
		reap "$pid"
		fail "authoritative active run did not resume wedge tracking"
	}
	reap "$pid"
	unset FM_FAKE_CREW_STATE
...
test_paused_authoritative_working_preserves_wedge_timer() {
	...
	dir=$(make_case paused-working-preserves-wedge-timer)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	...
	sig=$(seen_sig "$state/paused-working.status")
	printf '%s' "$sig" >"$state/.seen-paused-working_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle awaiting external")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '%s' "$pane_hash" >"$state/.stale-$key"
	...
	[ "$(cat "$state/.stale-since-$key" 2>/dev/null || true)" = "$since" ] ||
		{
			reap "$pid"
			fail "repeat authoritative working recheck reset the wedge timer"
		}
	reap "$pid"
...
test_wedge_escalation_marks_demand_deep_inspection_after_threshold() {
	...
	dir=$(make_case wedge-escalation)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	...
	sig=$(seen_sig "$state/wedged.status")
	printf '%s' "$sig" >"$state/.seen-wedged_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle building output")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher exited on the priming round (should absorb): $(cat "$out")"
	fi
	reap "$pid"
...
test_wedge_escalation_resets_when_pane_becomes_active() {
	local dir state fakebin out capture_file window key pane_hash sig pid
	dir=$(make_case wedge-escalation-reset)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	...
	sig=$(seen_sig "$state/wedged-reset.status")
	printf '%s' "$sig" >"$state/.seen-wedged-reset_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle building output")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher exited on a fresh (changed) pane hash: $(cat "$out")"
	fi
...
wedge_probe() { # <state> <age-secs> <escalations-recorded> [VAR=value...]
	...
	env FM_STATE_OVERRIDE="$state" FM_ROOT_OVERRIDE="$ROOT" "$@" bash -c '
		...
		. "$1"
		# Pin the clock this probe presents. Every assertion below sits on an exact
		# interval boundary, and reading the wall clock twice - once to date the
		# timer, once inside wedge_timer_check - lets a second tick over in between
		# and turns an age of interval-1 into interval (measured 8 slips in 200
		# probe calls on a loaded box, all of them reporting one second more than
		# the probe asked for). A pinned clock makes the boundary exact.
		probe_now=$(command date +%s)
		date() {
...
test_busy_pane_below_turn_age_bound_is_absorbed() {
	...
	dir=$(make_case busy-below-turn-age)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-busy-fresh"
	printf 'Working... (12.3s)' >"$capture_file"
	printf 'window=%s\nkind=ship\nharness=pi\n' "$window" >"$state/busy-fresh.meta"
	record_pi_busy "$state" busy-fresh
	printf 'working: setup complete\n' >"$state/busy-fresh.status"
	sig=$(seen_sig "$state/busy-fresh.status")
	printf '%s' "$sig" >"$state/.seen-busy-fresh_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "a busy pane below the turn-age bound was escalated: $(cat "$out")"
	fi
...
test_busy_pane_stable_hash_escalates_past_turn_age_bound() {
	local dir state fakebin out capture_file window key pane_hash sig pid
	dir=$(make_case busy-stable-hash-turn-age)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-busy-stable"
	printf 'Working...' >"$capture_file"
	printf 'window=%s\nkind=ship\nharness=pi\n' "$window" >"$state/busy-stable.meta"
	record_pi_busy "$state" busy-stable
	printf 'working: setup complete\n' >"$state/busy-stable.status"
	sig=$(seen_sig "$state/busy-stable.status")
	printf '%s' "$sig" >"$state/.seen-busy-stable_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "a stable-hash busy pane past the turn-age bound escalated before the wedge threshold: $(cat "$out")"
	fi
...
test_busy_pane_changing_hash_escalates_past_turn_age_bound() {
	...
	dir=$(make_case busy-changing-hash-turn-age)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-busy-ticking"
	...
	sig=$(seen_sig "$state/busy-ticking.status")
	printf '%s' "$sig" >"$state/.seen-busy-ticking_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "a changing-hash busy pane past the turn-age bound escalated before the wedge threshold: $(cat "$out")"
	fi
...
test_busy_pane_turn_end_touch_resets_age() {
	local dir state fakebin out capture_file window key pane_hash sig pid
	dir=$(make_case busy-turn-end-resets-age)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-busy-reset"
	printf 'Working...' >"$capture_file"
	printf 'window=%s\nkind=ship\nharness=pi\n' "$window" >"$state/busy-reset.meta"
	record_pi_busy "$state" busy-reset
	printf 'working: setup complete\n' >"$state/busy-reset.status"
	sig=$(seen_sig "$state/busy-reset.status")
	printf '%s' "$sig" >"$state/.seen-busy-reset_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "Working...")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "a freshly completed turn on a busy pane was still escalated: $(cat "$out")"
	fi
...
test_busy_pane_repeated_escalation_reaches_demand_deep_inspection() {
	local dir state fakebin out capture_file window key pane_hash sig pid n
	dir=$(make_case busy-turn-age-demand-inspect)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-busy-demand-inspect"
	printf 'Working...' >"$capture_file"
	printf 'window=%s\nkind=ship\nharness=pi\n' "$window" >"$state/busy-demand.meta"
	record_pi_busy "$state" busy-demand
	printf 'working: setup complete\n' >"$state/busy-demand.status"
	sig=$(seen_sig "$state/busy-demand.status")
	printf '%s' "$sig" >"$state/.seen-busy-demand_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "Working...")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "priming round for busy turn-age escalation was not absorbed: $(cat "$out")"
	fi
	reap "$pid"
...
test_busy_declared_pause_is_rechecked_not_wedge_escalated() {
	...
	dir=$(make_case busy-declared-pause)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-review-scout"
	...
	sig=$(seen_sig "$statusf")
	printf '%s' "$sig" >"$state/.seen-review-scout_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	...
	wait_poll_cycle "$state" "$pid" || {
		reap "$pid"
		fail "a declared pause on a busy review pane was escalated: $(cat "$out")"
	}
	reap "$pid"
	...
	wait_for_exit "$pid" 100 || {
		reap "$pid"
		fail "a declared pause past the long cadence was never rechecked"
	}
	...
	wait_poll_cycle "$state" "$pid" || {
		reap "$pid"
		fail "a lifted pause escalated before the wedge threshold: $(cat "$out")"
	}
	reap "$pid"
	...
	wait_for_exit "$pid" 100 || {
		reap "$pid"
		fail "a lifted pause on an over-age busy pane no longer wedge-escalates"
	}
...
test_busy_pane_default_turn_age_bound_is_3600s() {
	local dir state fakebin out capture_file window key pane_hash sig pid
	dir=$(make_case busy-default-turn-age)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-busy-default"
	printf 'Working...' >"$capture_file"
	printf 'window=%s\nkind=ship\nharness=pi\n' "$window" >"$state/busy-default.meta"
	record_pi_busy "$state" busy-default
	printf 'working: setup complete\n' >"$state/busy-default.status"
	sig=$(seen_sig "$state/busy-default.status")
	printf '%s' "$sig" >"$state/.seen-busy-default_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "Working...")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "a 5-minute-old completed turn tripped the default busy-turn-age bound: $(cat "$out")"
	fi
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "a 66-minute-old completed turn escalated before the wedge threshold under the default bound: $(cat "$out")"
	fi
...
test_nonterminal_stale_repairs_missing_or_corrupt_timer() {
	local dir state fakebin out capture_file window key pane_hash sig pid since
	dir=$(make_case nonterminal-stale-timer-repair)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	...
	sig=$(seen_sig "$state/quiet-timer.status")
	printf '%s' "$sig" >"$state/.seen-quiet-timer_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle building output")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	[ ! -s "$state/.wake-queue" ] || {
		reap "$pid"
		fail "missing stale-since repair enqueued a wake"
	}
	reap "$pid"
	...
	[ "$since" != "corrupt" ] || {
		reap "$pid"
		fail "corrupt stale-since value was left in place"
	}
	[ ! -s "$state/.wake-queue" ] || {
		reap "$pid"
		fail "corrupt stale-since repair enqueued a wake"
	}
	reap "$pid"
...
test_wedge_escalation_deferred_while_worktree_is_written() {
	...
	dir=$(make_case wedge-worktree-writes)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	capture_file="$dir/pane.txt"
	window="test:fm-writing"
	wt="$dir/wt"
	mkdir -p "$wt/src"
	printf 'idle building output' >"$capture_file"
	printf 'window=%s\nkind=ship\nworktree=%s\n' "$window" "$wt" >"$state/writing.meta"
	printf 'working: implementing\n' >"$state/writing.status"
	sig=$(seen_sig "$state/writing.status")
	printf '%s' "$sig" >"$state/.seen-writing_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle building output")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher wedge-escalated a quiet pane whose worktree was being written: $(cat "$out")"
	fi
	[ ! -s "$out" ] || {
		reap "$pid"
		fail "a written-worktree deferral printed a wake reason: $(cat "$out")"
	}
	[ ! -s "$state/.wake-queue" ] || {
		reap "$pid"
	...
	[ ! -e "$state/.wedge-escalations-$key" ] || {
		reap "$pid"
		fail "a deferral advanced the wedge escalation counter"
	}
	[ "$(cat "$state/.stale-since-$key" 2>/dev/null || echo 0)" -gt "$back" ] ||
		{
			reap "$pid"
			fail "a deferral did not restart the idle timer, so the next window cannot re-probe"
		}
	reap "$pid"
	ack_stopped_cycle "$state" || fail "could not acknowledge the intentional phase-A watcher stop"
...
test_write_deferral_resurfaces_on_the_bounded_cadence() {
	local dir state fakebin out drain_out capture_file window key pane_hash sig pid wt back
	dir=$(make_case wedge-worktree-resurface)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	...
	printf 'working: implementing\n' >"$state/churn.status"
	sig=$(seen_sig "$state/churn.status")
	printf '%s' "$sig" >"$state/.seen-churn_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle building output")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
...
test_secondmate_home_supervision_churn_is_not_write_evidence() {
	...
	dir=$(make_case secondmate-home-churn)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	...
	printf 'working: implementing\n' >"$state/mate.status"
	sig=$(seen_sig "$state/mate.status")
	printf '%s' "$sig" >"$state/.seen-mate_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
...
test_timer_repair_drops_a_finished_write_deferral_chain() {
	...
	dir=$(make_case wedge-write-chain-timer-repair)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-chain-repair"
	wt="$dir/wt"
	mkdir -p "$wt/src"
	printf 'idle building output' >"$capture_file"
	printf 'window=%s\nkind=ship\nworktree=%s\n' "$window" "$wt" >"$state/chain-repair.meta"
	printf 'working: implementing\n' >"$state/chain-repair.status"
	sig=$(seen_sig "$state/chain-repair.status")
	printf '%s' "$sig" >"$state/.seen-chain-repair_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "idle building output")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	[ ! -s "$state/.wake-queue" ] || {
		reap "$pid"
		fail "the idle-window timer repair enqueued a wake"
	}
	reap "$pid"
	...
	[ ! -s "$out" ] || {
		reap "$pid"
		fail "a fresh write deferral printed a wake reason: $(cat "$out")"
	}
	[ ! -s "$state/.wake-queue" ] || {
		reap "$pid"
...
test_terminal_first_sight_drops_a_finished_write_deferral_chain() {
	local dir state fakebin out capture_file window key pane_hash sig pid wt back
	dir=$(make_case wedge-write-chain-first-sight)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	capture_file="$dir/pane.txt"
	window="test:fm-chain-firstsight"
	wt="$dir/wt"
	mkdir -p "$wt/src"
	printf 'no-mistakes axi run: validating...' >"$capture_file"
	printf 'window=%s\nkind=ship\nworktree=%s\n' "$window" "$wt" >"$state/chain-first.meta"
	printf 'done: implementation complete, ready to validate\n' >"$state/chain-first.status"
	sig=$(seen_sig "$state/chain-first.status")
	printf '%s' "$sig" >"$state/.seen-chain-first_status"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	pane_hash=$(hash_text "no-mistakes axi run: validating...")
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "the overridden terminal status was not absorbed on first sight: $(cat "$out")"
	fi
...
SH
	...
	if ! wait_poll_cycle "$state" "$pid"; then
		reap "$pid"
		fail "watcher exited for a benign signal while testing log capping: $(cat "$out")"
	fi
	i=0
	while [ "$i" -lt 30 ]; do
	...
	[ "$lines" -le 2000 ] || {
		reap "$pid"
		fail "triage log was not capped when wc emitted a spaced byte count (lines=$lines)"
	}
	[ ! -s "$state/.wake-queue" ] || {
		reap "$pid"
		fail "benign signal enqueued a wake while testing log capping"
	}
	reap "$pid"
...
test_procevent_captured_result_surfaces_proactively() {
	...
	dir=$(make_case procevent-delivery)
	state="$dir/state"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
...
test_procevent_unacknowledged_result_redrains_until_handled() {
	...
	dir=$(make_case procevent-redrain)
	state="$dir/state"
	out="$dir/watch.out"
...
test_procevent_marker_keys_are_injective() {
	...
	dir=$(make_case procevent-marker-identity)
	state="$dir/state"
	out="$dir/watch.out"
...
test_procevent_surface_serializes_with_drain() {
	...
	dir=$(make_case procevent-drain-race)
	state="$dir/state"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	ready="$dir/marker-ready"
	release="$dir/marker-release"
...
test_procevent_surface_crash_boundaries() {
	...
	dir=$(make_case procevent-output-fail)
	state="$dir/state"
	out="$dir/watch.out"
	fifo="$dir/output.fifo"
	...
	dir=$(make_case procevent-before-marker)
	state="$dir/state"
	out="$dir/watch.out"
	append_wake "$state" check "procevent:before-marker:1" "check: procevent fixture before-marker 1"
	install_marker_mv_fault "$dir"
	FM_MARKER_MV_MODE=kill-before procevent_watch_bg "$dir" "$out"
	pid=$!
	wait_for_exit "$pid" 100
	exit_status=$?
	...
	dir=$(make_case procevent-after-marker)
	state="$dir/state"
	out="$dir/watch.out"
	append_wake "$state" check "procevent:after-marker:1" "check: procevent fixture after-marker 1"
	install_marker_mv_fault "$dir"
	FM_MARKER_MV_MODE=kill-after procevent_watch_bg "$dir" "$out"
	pid=$!
	wait_for_exit "$pid" 100
	exit_status=$?
...
test_procevent_marker_failure_exits_and_replays() {
	...
	dir=$(make_case procevent-marker-failure)
	state="$dir/state"
	out="$dir/watch.out"
...
test_heartbeat_no_change_absorbed() {
	...
	dir=$(make_case heartbeat-absorb)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
...
test_heartbeat_backstop_surfaces_unsurfaced_status() {
	...
	dir=$(make_case heartbeat-backstop)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
...
test_beacon_stays_fresh_while_absorbing() {
	...
	wait_poll_cycle "$state" "$pid" || {
		reap "$pid"
		fail "watcher exited while absorbing the first benign signal"
	}
	...
	wait_poll_cycle "$state" "$pid" || {
		reap "$pid"
		fail "watcher exited while absorbing a second benign signal"
	}
	...
	[ "$m2" -ge "$m1" ] || {
		reap "$pid"
		fail "beacon mtime regressed while absorbing"
	}
	[ "$((now - m2))" -lt 10 ] || {
		reap "$pid"
		fail "beacon went stale while absorbing (age $((now - m2))s)"
	}
	[ ! -s "$state/.wake-queue" ] || {
		reap "$pid"
		fail "absorbing benign signals enqueued a wake"
	}
	reap "$pid"
...
test_afk_paused_changed_pane_hands_off_plain_stale() {
	...
	dir=$(make_case afk-paused-changed-pane)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	...
	sig=$(seen_sig "$statusf")
	printf '%s' "$sig" >"$state/.seen-afk-held_status"
	date '+%s' >"$state/.afk"
	key=$(printf '%s' "$window" | tr '.:/' '___')
...
test_retired_task_alarms_stop_while_live_alarms_continue() {
	...
	dir=$(make_case retired-replay)
	state="$dir/state"
	fakebin="$dir/fakebin"
	out="$dir/watch.out"
	drain_out="$dir/drain.out"
	capture_file="$dir/pane.txt"
	live_capture="$dir/live-pane.txt"
	window="test:fm-w63"
	live_window="test:fm-live"
	key=$(printf '%s' "$window" | tr ':/.' '___')
	...
	printf 'working: implementing\n' >"$state/w63.status"
	sig=$(seen_sig "$state/w63.status")
	printf '%s' "$sig" >"$state/.seen-w63_status"
	printf '%s' "$pane_hash" >"$state/.hash-$key"
	printf '1\n' >"$state/.count-$key"
	PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
	...
	# Close phase 1's recovery episode the way a supervisor does between cycles,
	# so a later cycle supervises instead of replaying that handled wake.
	ack_stopped_cycle "$state" || fail "acknowledging the pre-retirement cycle failed"

	# Phase 3: a watcher cycle racing that teardown still queued one last alarm for
	# the retired pane. Rows already queued when a cycle arms are what make that
	# cycle announce its resurface, so the proof here is the DELIVERY: the drain
	# must drop the record naming the retired pane instead of alarming on it.
	append_wake "$state" stale "$window" "stale: $window" || fail "late in-flight append failed"
	FM_STATE_OVERRIDE="$state" "$DRAIN" >"$drain_out" 2>/dev/null || fail "drain of the in-flight record failed"
	grep -F "$window" "$drain_out" >/dev/null &&
		fail "a wake naming the retired pane was delivered after retirement: $(cat "$drain_out")"
	ack_stopped_cycle "$state" || fail "acknowledging the dropped in-flight record failed"

	# Phase 4: a genuinely live worker goes quiet on its own pane in the same home
	# and must still surface and still drain.
	printf 'window=%s\nkind=ship\n' "$live_window" >"$state/live.meta"
	printf 'working: implementing\n' >"$state/live.status"
	sig=$(seen_sig "$state/live.status")
	printf '%s' "$sig" >"$state/.seen-live_status"
