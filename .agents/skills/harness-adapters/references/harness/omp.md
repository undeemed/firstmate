...
| Multi-account | omp pools several accounts of the SAME provider natively, so a local gateway or router is never required to reach more than one account. Its own `providers` config plus `~/.omp/agent/models.yml` own the provider set, and `omp usage` reports each window. Verified 2026-09-21: with every gateway removed, `omp -p --model anthropic/claude-fable-5-1` answered directly while a gateway-prefixed id was rejected outright. A model id therefore names a real provider (`anthropic/...`), never a gateway route. |
