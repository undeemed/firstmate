...
1. **Run the updater:**
   ...
   bin/fm-update.sh
   ```
   It fast-forwards this firstmate repo's default branch from origin, then fast-forwards every registered secondmate home (a treehouse worktree of this same repo leased at a detached HEAD on the default branch, or a standalone clone fetched on its own) the same way.
