...
- `SWEEP: check <id>: <output>` / `SWEEP: reaped <id> (...)` / `SWEEP: left <id>: <reason>` / `SWEEP: orphan worktrees <pool>: <detail>` - the constant 3rd-mate sweep (`bin/fm-sweep.sh`) ran at session start (`docs/fleet-tiers.md`).
  A `check` line is the reap candidate's pending merge/check poll, run once before teardown removes it; treat it exactly like a `check:` wake - usually an externally merged PR - and relay it to the captain with the full PR URL.
  `reaped` means a landed/dead disposable crewmate was torn down (or, for the `orphan worktrees` form, a pool worktree was pruned); the sweep does none of the normal post-teardown bookkeeping, so for a reaped ship or scout task reconcile the backlog now - move the item to Done per `AGENTS.md` section 7 - and relay an externally-merged PR to the captain if its merge was first surfaced here.
