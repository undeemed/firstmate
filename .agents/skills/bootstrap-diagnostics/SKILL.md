...
description: >-
  ...
  Use whenever the session-start digest's bootstrap section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, CREW_DISPATCH invalid, FLEET_SYNC, PR_CHECK_MIGRATION, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SWEEP, NUDGE_SECONDMATES, or FMX - or when a standalone bin/fm-bootstrap.sh run prints one of those lines.
...
- `SWEEP: reaped <id> (...)` / `SWEEP: left <id>: <reason>` / `SWEEP: orphan worktrees <pool>: <detail>` - the constant 3rd-mate sweep (`bin/fm-sweep.sh`) ran at session start (`docs/fleet-tiers.md`).
  `reaped` means a landed/dead disposable crewmate was torn down (or, for the `orphan worktrees` form, a pool worktree was pruned); the sweep does none of the normal post-teardown bookkeeping, so for a reaped ship or scout task reconcile the backlog now - move the item to Done per `AGENTS.md` section 7 - and relay its outcome (a merged PR with its full URL, a scout report) to the captain if it was never relayed.
  A task whose merge poll (`state/<id>.check.sh`) is still armed is never swept, so a PR-ready task keeps its meta and its watcher merge wake until the captain's merge.
  A `left` line is the one worth reading: a reap candidate that teardown refused, usually a DEAD crew with unlanded work (the reason is quoted); decide whether that work should be recovered, pushed, or explicitly discarded before it is lost.
  Nothing prints when there was nothing to reap.
