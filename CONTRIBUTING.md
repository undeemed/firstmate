...
1. Fork this repo (`undeemed/firstmate`), then clone this repo or set your local `origin` back to it (`git@github.com:undeemed/firstmate.git`).
2. Create a branch and make your changes.
3. Initialize the gate with your fork of `undeemed/firstmate` as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate.git` (firstmate expects **no-mistakes v1.31.2+**; without a fork, plain `no-mistakes init` still works for maintainers with push access).
...
7. Once the pipeline passes, it pushes the branch to your fork and opens the PR against `undeemed/firstmate` for you.

Contributing upstream to `kunchenguid/firstmate` remains possible, but it is not this fork's default flow.
...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `docs/`, `tests/`, `.agents/skills/`, `skills/`, and the per-harness primary hook dirs (`.claude/`, `.codex/`, `.grok/`, `.opencode/`, `.pi/`).
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `docs/`, `tests/`, `.agents/skills/`, `skills/`, and the per-harness primary hook dirs - ship through the `no-mistakes` pipeline on a feature branch and require an explicit merge approval.
...
`--allow-empty-after-exclude` is for that pin only: when a branch's only mapped change is an excluded script, the runner says so on stdout and names the excluded families whose CI lane is the real gate instead of failing a legitimate change, while a hand-run exclusion that empties a real selection still exits 2.
