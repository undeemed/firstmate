...
tests/fm-turnend-guard.test.sh            # shared supervision predicate plus Claude Stop-hook scoping, loop guard, fail-open, live watcher health, and lock home/path filesystem-identity (symlink and case-variant spelling) tests
...
tests/fm-brief.test.sh                    # fm-brief.sh bash -n parse regression guard (issue #166), clean no-mistakes/direct-PR/local-only brief generation, and ship/scout peer-coordination paragraph tests
