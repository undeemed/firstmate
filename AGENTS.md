...
state/               volatile runtime signals; gitignored
  ...
  <id>.pi-ext.ts     fm-spawn-written per-task pi turn-end extension; removed by teardown
...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   When locked, the digest then also runs the constant 3rd-mate sweep once (`bin/fm-sweep.sh`, section 8) and relays its output as `SWEEP:` lines in the same bootstrap section.
...
If the resolved project has no secondmate yet, create its one persistent secondmate on demand (load `secondmate-provisioning`, section 6) and route the work there; only `local-only` and firstmate-repo work run without one (section 1 Fleet hierarchy).
