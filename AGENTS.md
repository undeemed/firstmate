...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, and `kimi`, plus `muse` and `omp` for crewmates and scouts only; never dispatch on an unverified adapter.
