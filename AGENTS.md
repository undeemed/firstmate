...
All three scaffolds - ship, scout, and the secondmate charter - also carry a compact peer-coordination paragraph that self-gates on a `herdr:` session-context line (inert on backends without one): a mate may see who else is working in the shared workspace and message a peer whenever it genuinely helps the work, not only on strict overlap or blockage, while firstmate authority and the status-file escalation protocol stay unchanged.
