...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `omp` for crewmates, scouts, and secondmates, and `muse` for crewmates and scouts only; never dispatch on an unverified adapter.
