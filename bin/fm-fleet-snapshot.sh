...
parent_evidence_reconciliation_json() {  # <summary-json> <activity-scan-json> <decisions-json>
  printf '%s\n' "$1" "$2" "$3" | jq -n '
    input as $summary | (input | .records) as $activities | input as $decisions |
...
secondmate_current_json() {  # <parent-tasks-json>
  ...
  local activity_scan decisions reconciliation provenance freshness reason summary summary_rc summary_bytes summary_valid summary_reason summary_invalidity state current_reason terminal terminal_contradiction contradiction
    ...
    | ([ $registered[] as $r
       ...
       + [ $tasks[] | select(.kind == "secondmate") as $t
           ...
           | {id:$t.id,home:($t.paths.home.path // null),
              ...
              parent_task:$t} ])
    | sort_by(.id)') || return 1
  total_registered=$(printf '%s' "$union" | jq '[.[] | select(.registered)] | length')
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      ...
      reconciliation=$(parent_evidence_reconciliation_json "$summary" "$activity_scan" "$decisions")
      ...
      record=$(printf '%s\n' "$summary" "$decisions" "$activity_scan" "$reconciliation" | jq -n \
        ...
        input as $summary | input as $decisions
        ...
        {id:$id,home:$home,host:($host | if . == "" then null else . end),remote:$remote,registered:$registered,
         ...
         parent_event:{raw:$event_raw,note:$event_note,age_seconds:$event_age,open_activities:$activity_scan.records,open_decisions:$decisions,activity_scan:$activity_scan,reconciliation:$reconciliation},
      ...
      record=$(printf '%s\n' "$activity_scan" "$decisions" | jq -n \
        ...
        input as $activity_scan | input as $decisions |
        {id:$id,home:($home | if . == "" then null else . end),host:($host | if . == "" then null else . end),remote:$remote,registered:$registered,
         ...
         parent_event:{raw:$event_raw,note:$event_note,age_seconds:$event_age,open_activities:$activity_scan.records,open_decisions:$decisions,activity_scan:$activity_scan},
         terminal_evidence:$terminal,contradiction:false}') || return 1
    fi
    # One JSON stream in memory, so the array is never rebuilt through argv.
    records+=$record$'\n'
  done <<EOF
$rows
EOF
  printf '%s\n' "$registry" "$records" | jq -n \
    ...
    'input as $registry | [inputs] as $records |
...
printf '%s\n' "$BACKLOG_JSON" "$TASKS_JSON" "$MAIN_INVENTORY_JSON" \
  ...
  'input as $backlog | input as $tasks | input as $main_inventory
   | input as $scout_reports | input as $secondmate_current
