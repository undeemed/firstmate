...
# outbound write is not attributable to a home after the fact. Each merge below
# therefore appends one tab-separated line to this home's own
# state/forge-write-audit.log BEFORE it is attempted, so a merge that crashes
...
#
# Only identity this script constructed is recorded - the home, the task id, the
# action, and the already-validated canonical URL - which is what structurally
# keeps a token, a header value, or any other secret out of the log. Reads are
# never recorded. When a second forge writer appears under bin/, lift these two
# functions into a shared library rather than writing a second log.
...
pipeline_merge_gate() {
  ...
  forge_audit pr-merge - "$URL" class=pipeline "head=$head" || {
