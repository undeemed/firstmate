...
#                       sweeps (same-home backlog reconciliation, firstmate
#                       instruction refresh, secondmate convergence, secondmate
#                       liveness, pending remote handoff retry, X-mode artifact
#                       writes, fleet sync) also run only when locked; the five
#                       network sweeps run in the deferred stage rather than this
...
#             sync, firstmate instruction refresh, same-home backlog
#             reconciliation, secondmate convergence and liveness, pending remote
#             handoff retry, X-mode artifact writes) - and
...
if [ "$READ_ONLY" -eq 1 ]; then
  printf 'skipped (read-only session) - GitHub authentication, firstmate instruction\n'
  printf 'refresh, project clone refresh, secondmate liveness and convergence, and pending\n'
  printf 'handoff delivery were not run.\n'
