...
# Primary config/omp-crew-overlay.yml likewise reaches a secondmate's own omp
# crewmates and scouts, never the secondmate itself (bin/fm-spawn.sh).
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode lavish-axi-host omp-crew-overlay.yml}"
