...
resurface_absorbed() {  # <window> <throttle-marker> <age> <reason> [interval] [scope]
  local win=$1 throttle=$2 age=$3 reason=$4 interval=${5:-$PAUSE_RESURFACE_SECS} scope=${6-}
...
handle_paused_stale() {  # <window> <task> <hash>
  local win=$1 task=$2 h=$3 key statusf mtime age interval detail reason declaration
  key=$(window_key "$win")
  ...
  resurface_absorbed "$win" "$STATE/.paused-resurfaced-$key" "$age" "stale: $win ($reason)" "$interval" "$declaration"
