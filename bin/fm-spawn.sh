...
case "$BACKEND" in
  tmux)
    ...
    # shell cd's into the worktree. WT_TARGET carries that stable id for the
    # rename-critical sends below; the persisted window= handle stays $T (the name
    # form), which is safe now that rename is disabled.
...
# #134 robustness: only tmux needs a worktree-send target distinct from $T - its
# rename-safe stable window id, set as WT_TARGET=$WID in the tmux branch above.
# Every other backend addresses its pane/surface by the id already in $T, so default
# WT_TARGET to $T for them (and for any future backend) - the shared worktree cd
