...
#  15. A same-basename self-home corr= line is restatement-copied onto the parent
#      channel and resolves, including after recovery delivery fails, instead of
#      escalating as a false miss
#  16. The mechanical helper writes the parent channel from (verb, corr, note)
#  17. Remote parent-replies.status is not classified as wrong-home
#  18. An escalated correlation stays retryable while undelivered, is never reset
