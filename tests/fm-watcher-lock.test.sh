...
test_guard_warnings() {
  ...
  # at the real home's config instead of this case's fixture.
...
test_watch_restart_reports_healthy_peer_without_attaching() {
  ...
  fakebin="$dir/fakebin"
  out="$dir/restart.out"
  node -e 'process.on("SIGTERM", () => {}); console.log("ready"); setTimeout(() => {}, 300000)' > "$dir/peer.out" &
  peer=$!
  # Wait until the peer's SIGTERM handler is actually installed ("ready" prints
  # only after process.on) - the restart below TERMs this pid, and a TERM landing
  # during node bootstrap would kill the deliberately TERM-resistant peer.
  i=0
  while [ "$i" -lt 50 ] && ! grep -q '^ready$' "$dir/peer.out" 2>/dev/null; do
    sleep 0.1
    i=$((i + 1))
  done
  grep -q '^ready$' "$dir/peer.out" 2>/dev/null || fail "peer never reported its TERM handler installed"
