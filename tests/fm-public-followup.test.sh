...
seed_commitment() {
  ...
  tasks_in "$home" public-followup add "$obligation" \
    ...
    --expected-final-file "$home/expected.json" --expires-at 2126-10-01T00:00:00Z >/dev/null \
...
# The seeded followup window is compared to the REAL clock on every path that
# does not pin FMX_NOW_OVERRIDE (rechain, pending escalation), so it must stay
# far-future; a near-future date here is a time bomb that detonates on expiry.
seed_repro_commitment() {   # <home> <obligation> <request> <work-home> <work-id>
  ...
  jq -n --arg r "$request" \
    '{request_id:$r, platform:"discord",
      ...
      followup_expires_at:"2126-08-28T01:12:00Z",
      reservation_expires_at:"2126-08-28T01:12:00Z"}' > "$home/request.json"
...
test_rechain_refuses_unclaimed_existing_destination() {
  ...
  tasks_in "$home" public-followup add public-final-existing-b \
    ...
    --expires-at 2126-08-28T01:12:00Z >/dev/null || fail "could not seed destination collision"
...
test_expiry_escalation_uses_now_override() {
  ...
  exp=$(date -u -j -f '%Y-%m-%dT%H:%M:%SZ' '2126-08-28T01:12:00Z' +%s 2>/dev/null) \
    || exp=$(date -u -d '2126-08-28T01:12:00Z' +%s)
