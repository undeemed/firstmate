...
test_stale_pool_base_refreshes_before_branching() {
	...
	# The pooled worktree is handed back out only after the first task's record is
	# gone; while that record stands, the occupied-checkout refusal owns this case
	# (tests/fm-spawn-worktree-collision.test.sh). Retire it as teardown would, so
...
strand_submodule_pin_via_spawn() {  # <seed-id>
  ...
  # The seed is done with the slot: retire its record as its cleanup would, so
  # the next spawn meets a free slot rather than an occupied-checkout refusal.
