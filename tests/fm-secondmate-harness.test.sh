...
SH
  ...
  # CLAUDECODE is scrubbed on every probe below: fm-harness.sh answers `claude`
  # on CLAUDECODE=1 before it ever looks at Pi's markers, so an ambient value
  # inherited from the shell that launched the suite (any Claude Code session)
  # makes all seven cases resolve `claude`. CI never exports it, so this only
  # ever failed locally.
  got=$(PATH="$fakebin:$BASE_PATH" CLAUDECODE='' PI_CODING_AGENT=true "$ROOT/bin/fm-harness.sh")
  [ "$got" = pi ] || fail "unmarked shared signed-wrapper ancestry resolved '$got', expected pi"
  got=$(PATH="$fakebin:$BASE_PATH" CLAUDECODE='' PI_CODING_AGENT=true FM_PI_HARNESS=pi-signed "$ROOT/bin/fm-harness.sh")
  [ "$got" = pi-signed ] || fail "selected signed wrapper resolved '$got', expected pi-signed"
  got=$(PATH="$fakebin:$BASE_PATH" CLAUDECODE='' PI_CODING_AGENT=true FM_PI_HARNESS=pi "$ROOT/bin/fm-harness.sh")
  [ "$got" = pi ] || fail "selected plain Pi resolved '$got', expected pi"
  got=$(PATH="$fakebin:$BASE_PATH" CLAUDECODE='' PI_CODING_AGENT=true FM_PI_HARNESS=pi-signed-helper "$ROOT/bin/fm-harness.sh")
  [ "$got" = pi ] || fail "inexact signed selection marker resolved '$got', expected pi"
  got=$(PATH="$fakebin:$BASE_PATH" CLAUDECODE='' FM_PI_HARNESS=pi-signed "$ROOT/bin/fm-harness.sh")
  [ "$got" = pi ] || fail "signed selection marker without Pi's family marker resolved '$got', expected pi"
  got=$(PATH="$fakebin:$BASE_PATH" CLAUDECODE='' PI_CODING_AGENT=true FM_TEST_SIGNED_SHAPE=plain "$ROOT/bin/fm-harness.sh")
  [ "$got" = pi ] || fail "plain Pi marker resolved '$got', expected pi"
  got=$(PATH="$fakebin:$BASE_PATH" CLAUDECODE='' PI_CODING_AGENT=true FM_TEST_SIGNED_SHAPE=helper "$ROOT/bin/fm-harness.sh")
...
# launch command (not just meta) can be asserted on. Also stubs treehouse so
# this same fakebin works for a crew/scout (non-secondmate) spawn, which leases
# its worktree with `treehouse get --lease` and must never reach the real
# binary from a test.
