...
phase_handoff() {
  ...
  # too old for atomic multi-ID mv (the handoff script refuses < 0.2.2; the
  ...
  if ! fm_tasks_axi_compatible; then
    echo "skip: tasks-axi on PATH lacks atomic multi-ID mv support (0.2.2+; backlog handoff delegates to it)"
    return 0
  fi
