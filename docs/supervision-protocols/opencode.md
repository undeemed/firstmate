...
4. A healthy watcher is silent: the plugin absorbs `watcher: healthy ...` arm output without prompting the session, so seeing that line (from a manual recovery probe) means do not start another cycle.
