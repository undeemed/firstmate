...
| omp | Run | `.omp/extensions/fm-primary-turnend-guard.ts` is the ported Pi transport: the same reason mapping, `session_compact` handling, and `pi.sendMessage` injection. `reload` stays unmapped. | The transport is STATIC EVIDENCE from omp's shipped declarations; no live omp session-start delivery has been verified yet (see the harness-adapters skill). |
