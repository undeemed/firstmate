...
| omp | Run | `.omp/extensions/fm-primary-turnend-guard.ts` is the ported Pi transport: the same reason mapping, `session_compact` handling, and message injection. `reload` stays unmapped. | The transport is STATIC EVIDENCE from omp's shipped declarations; no live omp session-start delivery has been verified yet (see the harness-adapters skill). |
...
Pi and its omp port are the only adapters that inject a message rather than hook stdout, so whatever they inject must carry operational provenance or the Ahoy skill would have to guess whether it was captain-authored.
...
It streams the hook to completion and retains at most 512 KiB for message delivery; this approved containment keeps the prefix and appends a loud `PI SESSION-START DELIVERY TRUNCATED` (or `OMP SESSION-START DELIVERY TRUNCATED`) marker with direct-inspection guidance whenever the digest is incomplete.
The omp port carries the same encoding, streaming, and truncation contract on omp's own transport.
