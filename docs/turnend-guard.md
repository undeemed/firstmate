...
The `fm_watcher_healthy` home/path halves compare filesystem identity (`fm_same_path` in `bin/fm-wake-lib.sh`: string equality, else same device+inode via `-ef`), not raw strings.
