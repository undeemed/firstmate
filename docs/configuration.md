...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned for the branch's newest run when axi status answers for another branch
...
FM_WATCH_WAKE_COALESCE_TTL_MS=300000   # omp adapter fail-open age at which an unread duplicate wake is re-delivered while the session has never observed a run consume queued input
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher first re-surfaces a declared external wait or verified captain-held transfer for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS; the away-mode daemon uses the same setting, on a flat cadence, for a declared external wait or verified captain-held transfer
...
FM_STEP_ACTIVITY_TIMEOUT=10        # wall-clock seconds the one bounded `no-mistakes axi status` read behind that bound may take, so a hung CLI cannot stall the watcher poll that started it; hitting the bound reads as no progress evidence, which leaves the escalation schedule exactly as it was
FM_STEP_CPU_SAMPLE_SECS=2          # sample width for the corroborating CPU read of a step's own agent process, which only enriches the recorded deferral reason and never decides an absorb
