...
omp is verified for crewmate and scout launches, and `fm-spawn.sh` also accepts it for a secondmate now that the primary supervision pair - the turn-end guard and the watcher bridge - is ported as tracked `.omp/extensions/*.ts` files loaded with `-e`; the [`harness-adapters` skill](../.agents/skills/harness-adapters/SKILL.md) owns that evidence and records which omp facts are still unverified.
...
For omp secondmate launches, `fm-spawn.sh` passes no `-e` at all: omp auto-discovers the home's tracked `.omp/extensions/` with no trust gate, and naming a discovered file with `-e` as well loads it twice, which [`fm-spawn.sh --help`](../bin/fm-spawn.sh) owns.
...
That deferred stage opens with the firstmate instruction refresh: a fast-forward-only update of this checkout from `origin`, so a landed instruction change reaches the home that is running and, through the secondmate sync below, every home that follows it.
It emits `FIRSTMATE_SYNC:` only when the loaded instruction surface actually advanced, which also names the `AGENTS.md` re-read the session now owes, when the checkout is behind `origin` and was left untouched, which reports how far behind it is and why it could not advance, or when `origin` could not be read, which reports that the drift cannot be measured rather than a behind count taken from the stale local origin ref.
A dirty, diverged, or off-default checkout is never forced, and a seeded secondmate home skips the step entirely because it follows the primary's commit rather than `origin`.
The same deferred network stage runs bootstrap's guarded tracked-file secondmate sync for recorded live homes, then propagates declared inherited local material into each validated live home under that sequencing contract.
...
FM_TASKTMP_ROOT=         # absolute parent directory for per-task scratch roots; default $XDG_CACHE_HOME/firstmate/tasktmp, then $HOME/.cache/firstmate/tasktmp (bin/fm-tasktmp-lib.sh)
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned for the branch's newest run when axi status cannot be attributed directly
...
FM_WATCH_WAKE_COALESCE_TTL_MS=300000   # unconditional omp adapter fail-open age after which a claimed unread wake stops suppressing delivery of the next wake
...
FM_PAUSE_RESURFACE_SECS=14400      # four hours before the watcher first re-surfaces a declared external wait or verified captain-held transfer for a recheck, including a live idle pane after its first inconclusive stale wake and a live busy pane past FM_BUSY_TURN_MAX_SECS; a structured `until <UTC ISO 8601>` time can make an external-wait recheck occur sooner but cannot extend this bound, a captain-held transfer is never rechecked while the away-posture record exists, and the away-mode daemon uses the same setting on a flat cadence, ageing its window against the crew's own latest status line rather than pane busy state
FM_PAUSE_BACKOFF_MAX_SECS=86400    # ceiling for the watcher's declared-wait recheck interval, which doubles as the unchanged declaration ages so the four-hour first recheck decays to about daily; a ceiling below FM_PAUSE_RESURFACE_SECS clamps to it, because the decay only ever slows a recheck down. A new declaration restarts the cadence, because the interval is derived from the declaration's own age
FM_SECONDMATE_WAKE_STALL_SECS=60   # minimum age of the oldest valid foreign wake-queue row before an endpoint-recorded local secondmate can produce a durable parent unattended-home or wake-loop-stall notification, and the same bound bin/fm-crew-state.sh uses before reading a mate with live children as unattended; zero or invalid values use 60
...
FM_SECONDMATE_WAKE_STALL_BEHIND_SECS=900  # oldest-row age at which a mid-turn mate is reported anyway, for the same reason; zero or invalid values use 900
FM_SECONDMATE_WAKE_STALL_REPEAT_SECS=300  # interval before an unchanged, still-behind backlog is reported again, doubling as its oldest row ages, so one report of the oldest row no longer silences a mate that stays behind; zero or invalid values use 300
FM_SECONDMATE_WAKE_STALL_REPEAT_MAX_SECS=3600  # ceiling for that repeat interval; zero or invalid values use 3600
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added
FM_WEDGE_BACKOFF_SECS=240          # defaults to FM_STALE_ESCALATE_SECS; the interval that doubles per further escalation once the same unchanged pane reaches FM_WEDGE_DEMAND_INSPECT_COUNT
FM_WEDGE_BACKOFF_MAX_SECS=3600     # defaults to FM_PAUSE_RESURFACE_SECS; ceiling for that growing interval, so a wedge still re-surfaces at least this often and is never silenced; a unit or ceiling below FM_STALE_ESCALATE_SECS clamps to it, because a backoff only ever slows a pane down
...
FM_STEP_ACTIVITY_MAX_SECS=900      # how long an ACTIVE pipeline step may go with no recorded activity before its quiet pane escalates as a possible wedge anyway; a progress bound, not a pacing knob (config/stale-escalate-secs still owns pacing), sized above the legitimate silence of a step waiting on an external service, such as a ci step waiting on GitHub checks; the age itself is read off the crew-state line that bin/fm-crew-state.sh already produces, bounded by FM_CREW_STATE_NM_TIMEOUT
FM_WATCH_TRIAGE_LOG_MAX_BYTES=262144   # size cap for the watcher and drain absorbed-wake debug log
...
FM_SPAWN_POOL_STATUS_TIMEOUT=15   # seconds allowed for fm-spawn.sh's treehouse pool-status read in the occupied-checkout check; an unreadable or timed-out pool falls back to the durable meta-record check alone
