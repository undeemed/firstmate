...
An absent `od` reports `MISSING: od (install: brew install coreutils  # or the platform's package manager)`; it ships with coreutils and is normally already present, but `bin/fm-dispatch-select.sh` needs it to break a tie between equally-available candidates, so bootstrap surfaces it here rather than letting a crewmate or scout intake meet it as a dispatch failure.
A determined selection - a single profile, a one-element array, or a single top scorer - consults neither `od` nor the random source, so an absent reader cannot block a dispatch whose outcome is already fixed.
