...
It emits `FIRSTMATE_SYNC:` only when the loaded instruction surface actually advanced, which also names the `AGENTS.md` re-read the session now owes, or when the checkout is behind `origin` and was left untouched, which reports how far behind it is and why it could not advance.
