...
Task selectors for `fm-peek.sh` resolve centrally through `fm_backend_resolve_selector`; `fm-send.sh` resolves the same vocabulary fail-closed from the underlying metadata helpers, refusing a bare window name with no recorded metadata, and `fm-crew-state.sh <id>` takes a task id directly with no selector resolution.
A selector containing `:` is an explicit backend endpoint escape hatch, passed through by `fm-peek.sh` and verified live (exiting non-zero otherwise) by `fm-send.sh`.
...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, od, no-mistakes v1.31.2 or newer, gh-axi, chrome-devtools-axi, lavish-axi, compatible tasks-axi per "Backlog backend" above, and quota-axi.
This section is the single owner of that universal toolchain list; backend guides' prerequisites point here and add only their backend-specific tools.
In that list, no-mistakes runs the validation pipeline, gh-axi, chrome-devtools-axi, and lavish-axi cover GitHub, browser, and rich-review operations, tasks-axi plus quota-axi back backlog mutations and quota-aware array dispatch, and od reads the OS random source that dispatch selection samples.
...
An absent `od` reports `MISSING: od (install: brew install coreutils  # or the platform's package manager)`; it ships with coreutils and is normally already present, but `bin/fm-dispatch-select.sh` needs it to break a tie between equally-available candidates and `fm_pid_identity` in `bin/fm-wake-lib.sh` reads the watcher's `/proc` command line through it on Linux - where an absent `od` makes every identity read fail, so the guards report a lapsed watcher chain for a live watcher - so bootstrap surfaces it here rather than letting a crewmate or scout intake meet it as a dispatch failure.
A determined selection - a single profile, a one-element array, or a single top scorer - consults neither `od` nor the random source, so an absent reader cannot block a dispatch whose outcome is already fixed, though the watcher-identity read has no such exemption.
...
FM_SWEEP_INTERVAL=300   # seconds between the watcher's detached constant 3rd-mate sweep launches (bin/fm-sweep.sh, docs/fleet-tiers.md)
FM_SWEEP_MAX_LINES=40   # sweep summary lines printed before the "... (N not shown)" cap
FM_SWEEP_PRUNE_ORPHANS=1   # set to 0 to skip the sweep's treehouse-prune orphan-worktree pass
FM_SWEEP_LOCK_STALE_SECS=600   # seconds before a leftover state/.sweep.lock is reclaimed as stale
FM_CONSULT_CODEX=codex  # codex binary used by fm-consult.sh consults, mainly for tests
FM_CONSULT_EFFORT=xhigh # reasoning effort for fm-consult.sh codex consults
FM_CONSULT_SANDBOX=read-only  # codex sandbox mode for fm-consult.sh consults; set danger-full-access only where the OS sandbox cannot run
FM_SPAWN_WT_WAIT_SECS=60   # seconds fm-spawn.sh waits for treehouse get to land the pane in an isolated worktree root
FM_CODEX_WATCH_CHECKPOINT=180   # seconds per foreground watcher checkpoint in Codex primary supervision
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per no-mistakes query inside fm-crew-state.sh
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status cannot be attributed to the current code
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage and the 3rd-mate sweep's idle-done gate
