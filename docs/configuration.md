...
That adapter, and only that adapter, retries the one exact transient response a cut-short listener returns while its marks remain available (`error: Lavish Editor poll response was interrupted` with `code: SERVER_ERROR`, followed only by the vendor's own `help[...]` diagnostics lines), up to 12 times with poll starts at least 5 seconds apart, so an internal retry never reaches the runner as a captured result.
...
Real feedback, ended and missing sessions, and any other `SERVER_ERROR` are captured and announced normally; `FM_LAVISH_POLL_RETRY_DELAY` is a bounded 1 to 60 second test override for the interval only, and the runner itself stays adapter-agnostic.
