...
An absent `od` reports `MISSING: od (install: brew install coreutils  # or the platform's package manager)`; it ships with coreutils and is normally already present, but `bin/fm-dispatch-select.sh` refuses to run without it, so bootstrap surfaces it here rather than letting every crewmate and scout intake meet it as a dispatch failure.
