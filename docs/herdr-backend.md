...
- **RESOLVED: worktree-discovery false-accept of the project clone under a case-variant home path.** Observed live 2026-07-06 (task `herdr-e2e-w3`) on a firstmate home reached via a differently-cased session cwd (`/users/xiao/dev/firstmate` for real case `/Users/xiao/Dev/firstmate`): meta recorded `worktree=/Users/xiao/Dev/firstmate/projects/herdr-e2e` (the project clone, true case) while `project=/users/xiao/dev/firstmate/projects/herdr-e2e` and the pane actually sat in the real treehouse worktree.
  On a case-insensitive filesystem (macOS APFS default), bash's `pwd -P` derives its answer from `$PWD` (symlink components resolved, TYPED case kept), while herdr's `foreground_cwd` and tmux's `pane_current_path` report the OS-level true-case cwd, so `PROJ_ABS_REAL` and the pane's unmoved starting cwd were two different strings for the identical directory.
  The very first worktree-discovery poll then saw "pane already moved", false-accepted the project clone as the acquired worktree, and recorded a wrong `worktree=` in the task meta; the crewmate itself still landed in the correct treehouse worktree, but teardown, review-diff, and every other meta consumer targeted the clone - the inverse failure direction of the symlinked-prefix entry above, from the same string-comparison root cause the 2026-07-06 canonicalization pass did not fully remove.
  Fixed 2026-07-06 (backlog `fm-spawn-case-guard-q2`): `bin/fm-spawn.sh`'s worktree-discovery poll and `validate_spawn_worktree` now compare filesystem identity (`[ a -ef b ]`, same device+inode) instead of canonicalized strings, keeping the single `PROJ_ABS_REAL` canonicalization; `-ef` follows symlinks itself, stays correct on case-sensitive filesystems where a lowercase-both comparison would wrongly merge two distinct directories, and is false for a nonexistent operand, matching the old never-equal string behavior (the poll's observed cwd names a live process's directory, so both operands exist on the paths that matter).
  This preserves both prior guarantees: a symlinked or case-variant prefix neither false-refuses an isolated spawn nor false-accepts the clone, and a genuinely tangled spawn (worktree resolving to the same physical directory as the project) still refuses via the same `-ef` identity that now also catches a case-variant tangle string comparison used to miss.
  Verified with GNU bash 3.2.57(1)-release (arm64-apple-darwin25) and git 2.50.1 (Apple Git-155) on macOS (Darwin 25.2.0): added `tests/fm-backend.test.sh:test_spawn_case_variant_project_path_waits_for_real_worktree`, which passes the project argument lowercased while the fake tmux's first `pane_current_path` poll returns the true-case project path and the second returns the real worktree (skipping itself on a case-sensitive filesystem via a marker-file probe).
  Confirmed the test reproduces the original bug against the pre-fix script (`git stash push -- bin/fm-spawn.sh` and rerun: `not ok - fm-spawn.sh should succeed for a project reached through a case-variant path` - the pre-fix poll false-accepts the clone as `WT` on its first reply and the spawn then fails downstream of that wrong worktree), and passes against the fix (`bash tests/fm-backend.test.sh` reports `ok - fm-spawn.sh: a project reached through a case-variant path (case-insensitive filesystem) does not false-accept the clone; discovery waits for the real worktree move`, with `test_spawn_symlinked_project_prefix_avoids_false_refusal` and the rest of the suite unaffected).
  `shellcheck bin/*.sh bin/backends/*.sh tests/*.sh` (ShellCheck 0.10.0) passes clean.
