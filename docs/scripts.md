...
| `fm-brief.sh`                      | Scaffold ship, scout, secondmate-charter, and Herdr-lab briefs; ship, scout, and the secondmate charter carry a self-gating peer-coordination note          |
| `fm-herdr-lab.sh`                  | Provision and guardedly operate an isolated, never-default Herdr lab session                                                                                |
| `fm-install-herdr.sh`              | Install CI's exact-version Herdr pin with official asset URL, SHA-256, and protocol checks                                                                  |
| `fm-install-treehouse.sh`          | Install CI's exact-version Treehouse pin for real-Herdr E2E that needs spawn worktrees                                                                      |
| `fm-herdr-ci-cleanup.sh`           | Snapshot and tear down only job-owned `fm-lab-*` sessions in the Herdr CI lane                                                                              |
| `fm-herdr-session-cleanup.sh`      | Retire this home's stale restored-shell Herdr presentation children at locked session start                                                                 |
...
| `fm-lint.sh`                       | Single owner of the lint definition (file set, config, pinned ShellCheck) that CI and the no-mistakes gate both run                                         |
| `fm-install-shellcheck.sh`         | Install the exact ShellCheck version `fm-lint.sh` pins                                                                                                      |
| `fm-doc-audience-check.sh`         | Validate documentation-audience inventory coverage, README setup routing, owner pointers, and local links (docs/documentation-audiences.md)                 |
...
| `fm-cd-command-policy.mjs`         | Semantic owner of the cd-guard PreToolUse policy, reusing the arm policy's shell lexer (docs/cd-guard.md)                                                   |
...
| `backends/herdr-eventwait.py`      | Wire transport only: subscribe to one herdr session's `pane.agent_status_changed` stream and print each event                                               |
| `backends/herdr-workspace-move.py` | Wire transport only: send one narrowly scoped `workspace.move` request and print the verified response                                                      |
...
| `fm-wake-lib.sh`                   | Shared durable wake queue, portable locks, filesystem path-identity (`fm_same_path`), and watcher identity/health helpers                                   |
| `fm-classify-lib.sh`               | Shared captain-relevant and declared-external-wait wake classification vocabulary                                                                           |
| `fm-transition-lib.sh`             | Shared backend-neutral agent-state transition shape and its single-owner supervision policy                                                                 |
| `fm-push-transition-lib.sh`        | Shared event-wait glue between a push-capable backend's transition stream and that policy                                                                   |
