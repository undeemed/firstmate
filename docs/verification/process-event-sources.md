...
## The published Opulent poll interface the adapter wraps
...
## Why an ended Opulent review is terminal
...
| one `Send & End`, one result | an armed Opulent source driven against a stand-in for the published poll, which delivers the final `session_ended` feedback once and empty ended sessions afterward, polls exactly once, captures exactly one result, publishes one distinct event, and retires itself |
...
| trusted classification boundary | Opulent lifecycle classification reads the leading response envelope, so prompt payload text that resembles a missing-session error cannot override a valid session status |
...
| canonical physical identity | a final-component symlink and its target produce the same Opulent source id |
...
Opulent is the first adapter; adding another requires only a new `bin/fm-procevent-<adapter>.sh`, whose `terminal` command is optional and defaults to keeping the source armed.
