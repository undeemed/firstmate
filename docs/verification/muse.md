...
The shared scrub that `bin/fm-spawn.sh` prepends to every locally sent launch command also clears `OMPCODE`, so an inherited omp marker cannot survive a Muse launch either.
