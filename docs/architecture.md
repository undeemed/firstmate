...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` with none of their own task worktree being written, their own pipeline step recording recent activity, or their own model turns completing, declared external waits and verified captain-held transfers that remain declared past their current recheck interval, and heartbeat backstop hits.
...
From that escalation onward the same unchanged pane re-surfaces on an interval that doubles from `FM_WEDGE_BACKOFF_SECS` up to `FM_WEDGE_BACKOFF_MAX_SECS`, so an already-reported wedge keeps reporting on a bounded cadence instead of waking the first mate every `FM_STALE_ESCALATE_SECS` forever.
Any genuine change - a changed pane hash, a busy pane, or a terminal or declared-pause status - clears the escalation count and returns the cadence to `FM_STALE_ESCALATE_SECS`.
...
That deferral re-surfaces on a flat `FM_PAUSE_RESURFACE_SECS` cadence, the interval a fresh declared wait starts at, with a reason naming the write evidence rather than a wedge, and it is bounded to one pruned, depth-bounded, wall-clock-bounded walk (`FM_WORKTREE_WRITE_PRUNE`, `FM_WORKTREE_WRITE_MAXDEPTH`, `FM_WORKTREE_WRITE_TIMEOUT`) taken only in the branch that was about to escalate, never on every poll.
...
A pane whose own pipeline step is active and recently active is deferred the same way, because a step waiting on an external service reads as silence to every liveness signal while the lane is healthy.
That deferral requires both conditions and never either alone, and both come off one authoritative current-state read: `bin/fm-crew-state.sh` must attribute an actively-running step to that crew's own code, and the `activity:` field it reports for that step - how long ago the step last recorded doing something - must be newer than `FM_STEP_ACTIVITY_MAX_SECS`.
Anything the progress probe cannot evaluate - no active step, an activity age past the bound, a run attributed to another branch, an unreadable or timed-out read - leaves the existing escalation schedule untouched.
A pane whose own model turns are still landing is deferred the same way, because that is the only evidence left for a lane running no pipeline at all: a `direct-PR` worker never has a step for the probe above to read, and a long turn spent reading and reasoning writes nothing for the worktree probe to find.
The evidence is `state/<id>.turn-ended`, the record firstmate's own per-harness turn-end hook touches at every turn boundary, so a marker newer than the start of the quiet window means the worker completed a model response and its tool calls behind the static pane.
A harness whose hook fires only when a whole turn ends records nothing mid-turn, which reads as no evidence and keeps the existing schedule, exactly as a missing marker, an older marker, or an unreadable one does.
...
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, every evidence deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
A crew that declared an external wait (`paused:`) or a verified captain-held transfer is the one exception to that bound: its busy verdict supplies liveness while identifying the long-running foreground call as the declared wait, so it takes the bounded declared-wait recheck instead of a wedge escalation.
...
A declared wait is the worker's own account of why its pane is quiet, so only an actively-running pipeline attributed to that crew's current code - positive evidence the crew resumed - outranks it, while a live crew whose authoritative state cannot name the wait at all still surfaces once.
A pane-sourced busy verdict does not outrank it, because a parked lane whose per-harness busy source keeps reporting busy is a reading of the very pane the declaration explains, and treating that as work in progress is what routed declared waits into the wedge escalator.
Rechecking an unchanged declaration carries no new information, so the recheck interval doubles as the declaration ages, starting at `FM_PAUSE_RESURFACE_SECS` and bounded by `FM_PAUSE_BACKOFF_MAX_SECS`: an hourly recheck for a fresh wait, a daily one for a wait that has held for days, and never silence.
The interval is derived from the declaration's own age rather than a counter, so a new declaration restarts the cadence and a watcher restart does not replay the ramp.
...
Once that row reaches `FM_SECONDMATE_WAKE_STALL_SECS`, the primary appends one keyed `check` wake naming the mate, row sequence, observed age, and queue depth, while empty and younger queues remain silent.
That wake names what is at risk rather than the loop that noticed it: when the mate's own home still holds live child task records, it reports an unattended home and how many child tasks are live, because the condition is then live work whose events nobody is consuming rather than a slow queue.
An aged row alone is not evidence that the mate's wake loop stalled: a mate that is mid-turn cannot reach its queue until that turn ends, so a mate whose endpoint is provably busy is given until `FM_SECONDMATE_WAKE_STALL_DEPTH` rows or `FM_SECONDMATE_WAKE_STALL_BEHIND_SECS` before it is reported at all.
Past either bound the mate is measurably behind and is reported however busy its endpoint looks.
Parent receipts and queued-key deduplication still prevent a storm and survive watcher and handling crashes, but they date the last report rather than vetoing every later one, so a backlog that stays behind repeats on an interval that doubles from `FM_SECONDMATE_WAKE_STALL_REPEAT_SECS` up to `FM_SECONDMATE_WAKE_STALL_REPEAT_MAX_SECS` instead of being silenced forever by the first report of its oldest row.
...
It warns rather than refuses, since workers append status with a plain `echo` and the fold deliberately honors the note-head position rather than losing a stated key, and a private per-task `.<id>.key-syntax-cursor` marker keeps each presented bad append to one report.
A row the section's byte cap omits is not recorded as warned: the task's marker advances only through the last row shown before the cut, so the omitted tail re-warns on the next drain instead of vanishing behind the omission count.
...
A local secondmate is read as the supervisor it is, because its own status log is truthful about the mate and silent about its tree: the live child task records in its home are part of the answer, so a mate with live children never reports idle or done, and one whose home has also left its own wake queue unconsumed past `FM_SECONDMATE_WAKE_STALL_SECS` while the mate is not mid-turn reports the distinct state `unattended`.
That read stays durable-record-only through `bin/fm-secondmate-home-lib.sh`, so a parent never reads a child's pane or reconstructs a secondmate's child tree.
...
The timer-refreshed fleet screens deliberately do not consume this snapshot; their shared cheap read layer is owned by [fleet-tui.md](fleet-tui.md).
...
Away-mode housekeeping has none of the watcher's evidence deferrals of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree, whose pipeline step is still recording progress, or whose model turns are still landing, escalates as a possible wedge at that bound.
...
The same boundary covers the brief's own source: a fresh ship or scout spawn measures the project clone against its remote in one bounded fetch and refuses while that clone is behind, because the brief was written from those files and only `bin/fm-fleet-sync.sh` may fast-forward a clone.
Its header owns the exact refusal mechanics, while `tests/fm-spawn-pool-base-freshen.test.sh` and `tests/fm-spawn-clone-base-drift.test.sh` own the portable regression coverage.
...
The record check runs as early as each spawn path's worktree is known - for a secondmate home before the pre-launch sync and inheritance propagation write into it, for crewmate and scout spawns after pool acquisition, and for Orca after worktree creation - and runs once more with the lease check just before metadata publication, under a home-scoped claim lock that spans that re-check to publication, so a refused spawn leaves the contested checkout byte-identical.
The record check covers only tasks recorded in this home, and a lease is in practice a secondmate home's claim, because firstmate leases only secondmate homes and never crewmate or scout worktrees.
An unleased crewmate or scout worktree recorded by another firstmate home sharing the same pool is invisible to both checks, so two homes on one pool can still collide; a durable pool-level claim visible to every home is the follow-up that would close that case.
...
Teardown reads the same durable claim records for the opposite reason: a process's cwd being inside a checkout proves ownership only while exactly one task records that checkout.
When another task also records it, teardown reaps only the processes its own records attribute to it, refuses by name when a process cannot be attributed at all, and leaves the shared checkout itself exactly as found; `bin/fm-teardown.sh`'s header owns that evidence and its refusal mechanics.
...
The parent guards every marked request against a missing correlated report without reading the secondmate conversation; `bin/fm-pending-reply-lib.sh` owns the correlation, recovery, escalation, settling, and retention contract.
...
Many homes share one forge credential and the forge cannot tell them apart afterwards, so the acting home's local record is the only thing that answers which home acted; [`bin/fm-forge-audit-lib.sh`](../bin/fm-forge-audit-lib.sh) owns the record and its format, and the record's own first line states that a direct `gh` invocation or a browser action is not captured.
...
The pipeline that opens a PR composes the published body itself, so a brief cannot reach that body by asking; a task that must publish specific content declares it with `bin/fm-brief.sh --pr-body-required`, and `bin/fm-pr-check.sh` publishes it as the body's final lines and confirms it by reading the published body back from the forge, refusing the task rather than recording a PR whose body silently lost it.
Because the pipeline publishes the worker's intent text verbatim, that same read-back also refuses to record a GitHub PR whose published body carries fleet-internal vocabulary (this firstmate repository is exempt because that vocabulary is its subject matter), keeping the disclosure out of a body that could later become public.
...
That implicit `--squash` is refused, naming the counts it read, when one live REST read of the pull request shows it carries too many commits or changed files to flatten without being asked, because a squash destroys a stack's ancestry permanently; an explicit merge method skips the guard and the script's header owns the exact thresholds.
