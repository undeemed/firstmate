...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past their current recheck interval, and heartbeat backstop hits.
...
A pane whose own pipeline step is active and recently active is deferred the same way, because a step waiting on an external service - a `ci` step waiting on GitHub checks - burns almost no CPU, writes no file, and renders nothing, so every liveness signal reads as silence while the lane is healthy.
That deferral requires both conditions and never either alone: the crew's authoritative current state must attribute an actively-running step to that crew's own code, and the step's own `last_activity` must be newer than `FM_STEP_ACTIVITY_MAX_SECS`, read once through one bounded `no-mistakes axi status` in the branch that was about to escalate.
A step agent that is also advancing CPU is recorded as corroboration in the deferral reason and never required for it, because progress, not liveness, is the question being asked.
