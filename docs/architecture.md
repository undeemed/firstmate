...
It warns rather than refuses, since workers append status with a plain `echo` and the fold deliberately honors the note-head position rather than losing a stated key, and a private per-task `.<id>.key-syntax-cursor` marker keeps each bad append to one report.
