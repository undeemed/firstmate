...
Rechecking an unchanged declaration carries no new information, so the recheck interval doubles as the declaration ages, starting at `FM_PAUSE_RESURFACE_SECS` and bounded by `FM_PAUSE_BACKOFF_MAX_SECS`: the shipped default rechecks a fresh wait after four hours and one that has held for days about daily, and never falls silent.
...
Rows that declare themselves a bounded external wait (`awaiting external - declared pause`) are not evidence of a stalled loop at all, because the pause cadence already owns their visibility.
...
A `blocked:` event reporting a refused or missing daemon socket outranks a potentially stale active run record; for other daemon, timeout, or unreachability claims, a running or fixing run with recent pipeline-reported activity supersedes the event and names reattachment as the recovery instead of surfacing a false block.
...
[`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh)'s header owns the exact branch, head, pipeline-custody, and newest-first attribution rules, and `bin/fm-crew-state.sh`'s header owns how this reader applies them.
...
Away-mode housekeeping has neither the worktree-write deferral nor the step-progress deferral of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree, or whose pipeline step is still recording progress, escalates as a possible wedge at that bound.
...
The parent guards every reply-bearing marked request against a missing correlated report without reading the secondmate conversation; `bin/fm-pending-reply-lib.sh` owns the correlation, recovery, escalation, settling, and retention contract, while `bin/fm-send.sh` owns the explicit fire-and-forget exception.
