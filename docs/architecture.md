...
A fourth bounded section, KEY SYNTAX, reports each newly appended decision line whose stated `[key=...]` token `fm-classify-lib.sh`'s `status_line_key_syntax_problem` says the fold could not use as written - a token past the note head folds under the shared `default` key, and a slug outside the charset makes the fold skip the line - because the printed note still shows the literal token and the loss is otherwise invisible until an answer is refused.
