...
On `attached` it stays live for exactly the one cycle it attached to and then unwinds, naming the healthy successor; it deliberately does not hop onto that successor, because the successor's own owning arm already carries the wake reason and a hopping arm never exits while supervision stays healthy.
An unexplained clean child close attaches for one such cycle in the same way, and an attached cycle that ends with no healthy successor at all becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
When a FAILED arm is explained by a live watcher from another checkout holding this home's `state/.watch.lock`, the line before the `FAILED` line names that holder's pid, recorded `watcher-path`, and recorded `fm-home`; arm refuses to clear or kill it, and [`watcher-continuity.md`](watcher-continuity.md) owns that diagnostic's contract.
