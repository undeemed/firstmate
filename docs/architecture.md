...
A declared wait is the worker's own account of why its pane is quiet, so it holds while the agent is alive and outranks every liveness read except an actively-running pipeline attributed to that crew's current code.
