...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, `gemini`, `muse`, `rovo`, and `omp`, plus Secondmate spawns across that same set except the deliberately crewmate-only `gemini`, `muse`, and `rovo` adapters.
