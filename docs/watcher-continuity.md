...
The omp adapter also coalesces byte-identical unread wakes, so a repeating actionable line queues one follow-up until the run consumes queued input; the coalescing contract lives in the `.omp/extensions/fm-primary-omp-watch.ts` header comment, with the fail-open bound configured through `FM_WATCH_WAKE_COALESCE_TTL_MS`.
...
The same adapter delivers a wake raised while the session is idle without a delivery mode, which starts the turn, and keeps `followUp` only while a run is streaming, because omp's idle-path auto-continue for a queued follow-up is gated on the transcript tail and on no prior interrupt.
...
Before the run consumes queued input, omp coalesces all pending watcher wakes for its generation into one follow-up.
...
`tests/fm-omp-watch-extension.test.sh` covers the omp port: `fm_watch_arm_omp` tool and `/fm-watch-arm-omp` command identity, first-cycle arm start, the owned wake for an externally healthy watcher, session-generation retirement on shutdown, and coalescing of byte-identical unread duplicate wakes into one queued follow-up that is delivered again once queued input is consumed.
