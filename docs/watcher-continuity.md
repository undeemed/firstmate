...
Pi and omp same-process session replacement follows the generation-owner contract in `.pi/extensions/fm-primary-pi-watch.ts` and its omp port in `.omp/extensions/fm-primary-omp-watch.ts`.
