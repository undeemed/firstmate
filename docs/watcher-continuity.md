...
Pi's `.pi/extensions/fm-primary-pi-watch.ts`, omp's ported `.omp/extensions/fm-primary-omp-watch.ts`, and OpenCode's `.opencode/plugins/fm-primary-watch-arm.js` own continuous re-arm after an actionable child close.
...
The omp adapter also keeps one pending unread wake per session generation, so a burst of actionable lines yields one delivery until the run consumes queued input; the coalescing contract lives in the `.omp/extensions/fm-primary-omp-watch.ts` header comment, with the fail-open bound configured through `FM_WATCH_WAKE_COALESCE_TTL_MS`.
That bound is unconditional, and a run that settles without consuming the queued wake releases the claim, so a follow-up omp dropped when a turn aborted cannot suppress every later wake.
Pi and omp same-process session replacement follows the generation-owner contract in `.pi/extensions/fm-primary-pi-watch.ts` and its omp port in `.omp/extensions/fm-primary-omp-watch.ts`: an owning `session_start` arms the replacement generation without waiting for a model turn, and a state-scoped replacement handoff carries every actionable close whose delivery overlapped `session_shutdown`, including a main follow-up Pi accepted but had not yet consumed, branch handling, and a retiring child that reports after the bounded shutdown wait.
...
Before the run consumes queued input, omp coalesces all pending watcher wakes for its generation into one delivery.
The durable wake queue preserves every underlying event, so coalescing notification text cannot drop a wake.
...
`state/.watch-triage.log` remains only the bounded absorbed-wake debug log and carries no lifecycle semantics.
...
`tests/fm-omp-watch-extension.test.sh` covers the omp port: `fm_watch_arm_omp` tool and `/fm-watch-arm-omp` command identity, first-cycle arm start, the owned wake for an externally healthy watcher, session-generation retirement on shutdown, coalescing of duplicate and distinct unread wakes into one delivery that repeats once queued input is consumed, expiry of a claimed wake whose queued follow-up was lost once the coalesce TTL passes, and the wake-delivery lifecycle: an idle wake starts a turn, a streaming wake queues as a follow-up, a run that settles without consuming the wake releases the claim, and a scheduled continuation keeps it.
...
The omp port has deterministic test coverage only and is not yet part of that live evidence.
